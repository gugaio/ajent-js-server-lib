...
const client = LLMFactory.createGeminiClient('ajent-445416');
 ...
 */
function createGeminiApiServer(options = {}) {
  const {
    app: existingApp,
    port = 3000,
    uploadDir = './uploads',
  ...
  const defaultLlmProject = options.llmProject;
  const defaultLlmName = options.llmName || 'gemini';
  const defaultLlmModel = options.llmModel || 'gemini-2.5-flash-preview-05-20';
...
module.exports = createGeminiApiServer;